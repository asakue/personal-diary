...
const SidebarProvider = React.forwardRef((_a, ref) => {
    ...
    return (<SidebarContext.Provider value={contextValue}>
        <tooltip_1.TooltipProvider delayDuration={0}>
          <div style={Object.assign({ "--sidebar-width": SIDEBAR_WIDTH, "--sidebar-width-icon": SIDEBAR_WIDTH_ICON }, style)} className={(0, utils_1.cn)("group/sidebar-wrapper flex min-h-svh w-full text-sidebar-foreground has-[[data-variant=inset]]:bg-sidebar", className)} ref={ref} {...props}>
            {children}
          </div>
